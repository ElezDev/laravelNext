...
import { getRoles , getPermissions, getPermissionsForRole, assignPermissionsToRole } from './roles.service';
  ...
  useEffect(() => {
    if (selectedRole) {
      const fetchPermissionsForRole = async () => {
        try {
          const permissionsForRole = await getPermissionsForRole(selectedRole);
    ...
    <PrincipalLayout>
      <div className="">
        ...
        <div className="mb-4 w-full bg-white shadow-md rounded-lg p-4">
          <h2 className="text-lg font-medium mb-2 text-gray-800">Permisos</h2>
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Permiso</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Descripción</th>
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Asignar</th>
              </tr>
            </thead>
                    ...
                    <label className="flex items-center">
                      ...
                      <div
                        className={`relative h-6 w-12 rounded-full bg-gray-200 flex items-center p-1 cursor-pointer transition-colors duration-200 ease-in-out ${
                          selectedPermissions.includes(permission.name) ? 'bg-indigo-600' : 'bg-gray-200'
                        }`}
                      >
                        <div
                          className={`absolute h-5 w-5 rounded-full ${
                            selectedPermissions.includes(permission.name) ? 'transform translate-x-6 bg-white' : 'transform translate-x-0 bg-white'
                          }`}
                        />
                      </div>
                    </label>
